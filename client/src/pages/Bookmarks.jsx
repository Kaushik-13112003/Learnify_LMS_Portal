...
const Bookmarks = () => {
  ...
  const { data: courses, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await fetch("/api/user/my-bookmarked-course", {
          method: "GET",
        });
  ...
  return (
    <>
      <div className="p-10">
        <h1 className="text-3xl font-bold mb-6 text-center">My Bookmarkes</h1>
        {allCourses?.length <= 0 && (
          <div className="mt-5 h-screen flex flex-col gap-3 items-center">
            <p>you haven't bookmarked any course yet</p>
            <NavLink
              to="/courses/all"
              className={
